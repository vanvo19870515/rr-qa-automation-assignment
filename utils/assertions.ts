...
export function expectWithinRange(value: number, min: number, max: number, context: string): void {
  expect(value, `${context}: expected ${value} to be between ${min} and ${max}`).toBeGreaterThanOrEqual(min);
  expect(value, `${context}: expected ${value} to be between ${min} and ${max}`).toBeLessThanOrEqual(max);
}
