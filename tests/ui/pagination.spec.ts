...
test('TC16 – Navigate to page 2 @smoke', async ({ homePage, page, log }) => {
  ...
  await test.step('Verify page 2 content differs', async () => {
    const page2Titles = await homePage.results.titles();
...
test('TC17 – High page numbers beyond API limit @defect', async ({ homePage, page, log }) => {
  ...
  await test.step('Click high page — expect error', async () => {
    ...
    if (await highBtn.isVisible({ timeout: 2_000 }).catch(() => false)) {
      await highBtn.click();
      await Promise.race([
        homePage.results.errorMessage.waitFor({ state: 'visible', timeout: 5_000 }),
        page.waitForResponse((r) => r.status() >= 400, { timeout: 5_000 }),
      ]).catch(() => {});
      const hasError = await homePage.results.hasError();
